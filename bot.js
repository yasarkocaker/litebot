...
client.on("message", msg => {
  if (msg.content.toLowerCase() === "sa") {
    msg.reply("Esselamu aleyküm ve rahmetullahı berekatüh mümmin kardeşim");
  }
});
...
client.on("message", async message => {
  ...
  client.channels.cache.get("645670665784918017").send(
...
client.on("message", async message => {
  ...
  if (command === "sunucu-kurulum") {
    if ( message.guild.channels.cache.find(channel => channel.name === "Bot Kullanımı")
    )
      return message.channel.send("Sunucu Zaten Ayarlanmış");
    message.channel.send(
      `Bot Bilgi Kanallarının kurulumu başlatılsın mı? başlatılacak ise **kabul** yazınız.`
    );
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
        " Bu Kodu `Yönetici` Yetkisi Olan Kullanabilir"
      );
    message.channel
          ...
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
                channel => channel.name === "✮ ▬  ▬ Duyuru Kanalları▬  ▬ ✮"
              )
            )
          );
        message.guild
          .channels.create("「💚」gelen-giden", "text", [
            {
              id: message.guild.id,
              deny: ["SEND_MESSAGES"]
            }
      ...
      .then(collected => {
        ...
        message.guild
          .channels.create(`【🎁】şikayet-ve-öneriler`, "text")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
        ...
        message.guild
          .channels.create(`【👥】video-duyurular`, "text")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
                channel => channel.name === "✮ ▬  ▬ Metin Kanalları▬  ▬ ✮"
        ...
        message.guild
          .channels.create(`【📷】galeri-odası`, "text")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
                channel => channel.name === "✮ ▬  ▬ Metin Kanalları▬  ▬ ✮"
        ...
        message.guild
          .channels.create(`【🤖】bot-komut`, "text")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
                channel => channel.name === "✮ ▬  ▬ Metin Kanalları▬  ▬ ✮"
        ...
        message.guild
          .channels.create(`【👻】sohbet-odası`, "text")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
                channel => channel.name === "✮ ▬  ▬ Metin Kanalları▬  ▬ ✮"
        ...
        message.guild
          .channels.create(`🌹》Kurucu Odası`, "voice")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
        ...
        message.guild
          .channels.create(`👍》Sesli Yönetici Odası`, "voice")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
                channel => channel.name === "|▬▬|Ses Kanalları|▬▬|"
        ...
        message.guild
          .channels.create(`💬》Sesli Sohbet Odası`, "voice")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
                channel => channel.name === "|▬▬|Ses Kanalları|▬▬|"
        ...
        message.guild.owner.send(
          "Gerekli Herşey Kuruldu Rahatına Bak! **BlackBlvee'un kıymetini bil :D**"
        );
      });
  }
});
    ...
    member.guild.channels
      ...
      .send(
        `${member} Kullanıcısına \`${
          member.guild.roles.cache.get(rol).name
        }\` rolü verildi! **${member.guild.members.cache.size}** Kişiyiz!`
      );
  }
});
...
client.on("message", async message => {
  ...
  if (command === "rol-kur") {
    ...
    message.channel.send(
      `Bot Gerekli Rollerin kurulumu başlatılsın mı? başlatılacak ise **evet** yazınız.`
    );
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
...
client.on("message", async message => {
  // bot bilgi paneli üye sayısı bot sayısı falan
  const ms = require("ms");
  const args = message.content
    .slice(prefix.length)
    .trim()
  ...
  if (command === "serverpanelkaldır") {
    if (
      !message.guild.channels.cache.find(channel => channel.name === "Server Panel")
    )
      return message.channel.send("**Server Panel Ayarlanmamış!**");
    if (!message.member.hasPermission("ADMINISTRATOR"))
    ...
    const b = message.guild.channels.cache.find(
      channel =>
        ...
        } bot / ${message.guild.memberCount} üye`,
      true
    );
    ...
    const d = message.guild.channels
      ...
      .delete(); //|| message.guild.channels.cache.find(channel => channel.name === `Kanal sayısı: ${message.guild.channels.size-1}`).delete() || message.guild.channels.cache.find(channel => channel.name === `Kanal sayısı: ${message.guild.channels.size-1}`).delete() || message.guild.channels.cache.find(channel => channel.name === `Kanal sayısı: ${message.guild.channels.size-2}`).delete()
    if (!d) return console.log("guildStatsChannel");
    message.channel.send("**Kanallar Temizlendi!**");
  }
 if (command === "serverpanel") {
    if (message.guild.channels.cache.find(channel => channel.name === "Server Panel"))
      return message.channel.send(" Bot Paneli Zaten Ayarlanmış.");
    message.channel.send(
      `**Server Panel Odalarının Kurulumunun Başlamasını İstiyorsanız 'başlat Yazınız!'**`
    );
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(" Yetkin bulunmuyor.");
    message.channel
      .awaitMessages(response => response.content === "başlat", {
        max: 1,
        time: 10000,
        errors: ["time"]
      })
      .then(collected => {
        message.guild.channels.create("Server Panel", "category", [
          {
            id: message.guild.id,
            deny: ["SPEAK"],
            deny: ["CONNECT"]
        ...
        message.guild
          .channels.create(`Toplam Üye • ${message.guild.memberCount}`, "voice")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
        ...
        message.guild
          .channels.create(
            `Çevrimiçi Üye • ${
              client.users.cache.filter(cfx => cfx.presence.status === "online").size
            }`,
            "voice"
          )
          .then(channel =>
        ...
        message.guild
          .channels.create(
            `Botlar •  ${message.guild.members.cache.filter(m => m.user.bot).size}`,
            "voice"
          )
          .then(channel =>
            channel.setParent(
        ...
        message.guild
          .channels.create(`Rekor Online • Bakımda!`, "voice")
          .then(channel =>
            channel.setParent(
              message.guild.channels.cache.find(
                channel => channel.name === "Server Panel"
    ...
    if (dakdest !== null && timeout - (Date.now() - dakdest) > 0) {
      ...
      if (message.content.length > 1) {
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setTitle("Gold Üye!")
          .setDescription(
            "<a:krst:627824669201793024> **Bir gold üye belirdi!**"
          );
        ...
        client.on("message", async message => {
          ...
          if (!message.guild) return;
          if (
            message.content === "<@id570090430855643168>" ||
            message.content === "<@!id570090430855643168>"
             ) {
            message.author.send("atilicak mesaj");
          }
        });
      }
    }
